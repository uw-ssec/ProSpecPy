...
Minimum input:
    A directory containing Original ProSpecPy baseline outputs with files named
    baseline_corrected_data.csv and, optionally,
    baseline_corrected_peak_info.csv.
...
def parse_args() -> argparse.Namespace:
    ...
    input_group.add_argument(
        ...
        type=Path,
        help=(
            "Directory containing Original ProSpecPy anchor+spline outputs to scan recursively. "
    ...
    input_group.add_argument(
        ...
        help=(
            ...
            "Use this for comparison against the Original ProSpecPy anchor+spline method."
        ),
    )
    input_group.add_argument(
        "--run-native-original",
        action="store_true",
        help="Optionally run the Original ProSpecPy anchor-point + spline workflow from raw pH6 data first.",
    )
    input_group.add_argument(
        "--workflow-root",
        type=Path,
    ...
    original_group.add_argument(
        "--config-source",
        default="workflow_demo.ipynb original anchor+spline workflow",
    )
    original_group.add_argument("--peak-threshold", type=float, default=0.35)
    original_group.add_argument(
        "--adjustment-factor",
        type=float,
        default=1.0,
        help="Adjustment factor used by anchor_point_fit().",
    )
    original_group.add_argument("--spline-degree", type=int, default=3)
    original_group.add_argument("--spline-smooth", type=float, default=0.0)
    original_group.add_argument("--range-start", type=int, default=2150)
    original_group.add_argument("--range-end", type=int, default=1850)
...
def baseline_config(args: argparse.Namespace) -> str:
    config = {
        ...
        "negative_values_clipped_to_zero": True,
    }
...
def run_native_original(
    ...
    for obj in spectra:
        ...
        obj.anchor_point_fit(args.adjustment_factor)
        obj.baseline_fit()
        obj.subtract_baseline(save=True, showplot=False, verbose=False)
    metadata = {
    ...
    (native_output / "original_baseline_run_metadata.json").write_text(
...
def prepare_erni_inputs(
    ...
    for source in source_files:
        ...
        base_run_id = f"original-anchor-spline-{slug(sample_id)}"
        run_id = base_run_id
        suffix = 2
        ...
        candidate_peak_file = ""
        peak_source = source.with_name("baseline_corrected_peak_info.csv")
...
def make_plots(
    ...
    *,
    ...
    for label, attempt in (("best_fit", good), ("review_residual", review)):
        ...
        axes[0].set_title(
            f"Sample {attempt['sample_id']}: Original anchor+spline Gaussian {label.replace('_', ' ')}"
        )
    ...
    captions = [
        "The downstream wrapper can consume Original ProSpecPy anchor+spline-corrected spectra and export valid peak parameters.",
        "Residual plots are included to separate rule-level validity from residual-pattern review.",
    ]
    if control_metadata is not None:
        captions.insert(
            0,
            "Control and Original ProSpecPy anchor+spline corrected spectra are plotted for matched samples when available.",
        )
...
def main() -> None:
    ...
    native_output = args.output_dir / "original_anchor_spline_native_outputs"
    ...
    comparison_output = args.output_dir / "control_vs_original_anchor_spline_comparison_summary.csv"
    plot_dir = args.output_dir / "report_plots"

    if args.run_native_original:
        print("Running Original ProSpecPy anchor+spline workflow...")
        run_native_original(args, native_output)
    ...
    print_counts("Original ProSpecPy anchor+spline", erni_fit_dir)
    ...
    if args.control_metadata is not None:
        ...
        write_comparison(
            ...
            erni_fit_dir,
            label_a="control",
            label_b="original_anchor_spline",
        )
